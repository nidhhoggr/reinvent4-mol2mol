...
Reads the scalar tags from a single REINVENT TensorBoard run directory and
writes one wide CSV (one row per step, one column per metric), plus a
first/last/min/max/delta summary to stdout.
"""
import csv
import os
import sys
...
def merge_dirs(run_dirs):
    """Combine scalars from multiple run dirs by step (last write wins)."""
    merged = {}  # tag -> {step: value}
    for rd in run_dirs:
        for tag, series in load_run(rd).items():
            merged.setdefault(tag, {}).update(series)
    return merged
...
def main():
    ...
    data = load_run(runs[0]) if len(runs) == 1 else merge_dirs(runs)

    # Events file can exist before any scalar is flushed -> no tags/steps yet.
    # Leave any prior tb_stats.csv intact and exit cleanly so the loop retries.
    if not any(series for series in data.values()):
        sys.exit("no scalars flushed yet; nothing to write")

    out = os.path.join(outdir, "tb_stats.csv")
    tags, steps = write_wide_csv(out, data)

    label = runs[0] if len(runs) == 1 else f"{root} ({len(runs)} dirs)"
    print(f"\n{label} -> {out}")
